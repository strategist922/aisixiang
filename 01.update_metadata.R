...
N <- 13 # 该月点击排行总页数
Url <- paste0("http://www.aisixiang.com/toplist/index.php?id=1&period=30&page=", 2:N)
for(i in Url){
  Update <- rbind(Update, Download(i))
  print(i)
}

Aisixiang <- read_csv("aisixiang_2016-12-27.csv") # 读入前一次更新的存档
Aisixiang_new <- rbind(Aisixiang, Update)
Aisixiang_new <- unique(Aisixiang_new)

write.csv(Aisixiang_new, "aisixiang_2017-01-20.csv", row.names = F) # 注意命名
